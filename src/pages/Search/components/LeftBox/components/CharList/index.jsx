...
const CharList = (props) => {
  ...
  function requestAndShowCharInfoByKey(char, infoKey, callback) {
    queryCharInfo({
      ...
      .then((result) => {
        const { data } = result;
        let infoStr = formatShuowenText(data?.[0]?.[infoKey] ?? "", infoKey);
        callback();
        notification.open({
          message: false,
          duration: false,
  ...
  function rowRenderer({ index: infoIndex, key, parent, style }) {
    ...
    if (infoIndex === 0) {
      itemElement = (
        <div>
          <div className={styles.char_list_top_info}>
            <div
            ...
            <div className={styles.char_btns}>
              <Spin spinning={uniCodeLoading} size="small">
                <div
                  className={styles.char_unicode}
                  onClick={() => handleUnicodeClick(charInfo.char)}
                >
              ...
              <Spin spinning={shuowenLoading} size="small">
                <div
                  className={styles.char_shuowen}
                  onClick={() => handleShuowenClick(charInfo.char)}
                >
                  说文
                </div>
              </Spin>
              <Spin spinning={kangxiLoading} size="small">
                <div
                  className={styles.char_kangxi}
                  onClick={() => handleKangxiClick(charInfo.char)}
                >
                  康熙
                </div>
              </Spin>
              <Spin spinning={huizuanLoading} size="small">
                <div
                  className={styles.char_huizuan}
                  onClick={() => handleHuizuanClick(charInfo.char)}
                >
                  汇纂
                </div>
              </Spin>
              <Spin spinning={handaLoading} size="small">
                <div
                  className={styles.char_handa}
                  onClick={() => handleHandaClick(charInfo.char)}
                >
                  汉大
                </div>
              </Spin>
              <div
                className={styles.char_map}
                onClick={() => handleMapClick(charInfo.char)}
              >
                🌎️
              </div>
            </div>
  ...
  useEffect(() => {
    // 搜索展示时默认跳过为空的项
    let hasValIndex = searchData.findIndex(
      (item) => Object.keys(item.charInfo).length > 0
    );
  ...
  return (
    <>
      ...
      >
        <div
          className={
            isPad ? styles.char_list_box_left_mobile : styles.char_list_box_left
          }
        >
          ...
          {searchData.map((charItem) => {
            return (
              <Button
                color={
                  `${charItem?.char}_${charItem?.originChar}` ===
                  `${selectedCharItem?.char}_${selectedCharItem?.originChar}`
                    ? "primary"
                    : "default"
                }
        ...
        <div
          className={
            isPad
              ? styles.char_list_box_right_mobile
              : styles.char_list_box_right
          }
        >
          {selectedCharInfos &&
          Array.isArray(selectedCharInfos) &&
          selectedCharInfos.length > 0 ? (
            <>
              <div
                style={{
                  boxSizing: "border-box",
